...
const actions = fetchActions("building.update");

const BuildingUpdateRedux = {
	update: function (building) {
		return (dispatch, getState) => {
			dispatch(LoadingRedux.start());
			dispatch(actions.request());
			return Server.post(DiscoveryRedux.selector.root.building.update(getState()), building)
				.then(({data}) => {
	...
	selector: fetchSelector(state => BuildingRedux.selector.branch(state).update)
};

export default BuildingUpdateRedux;
